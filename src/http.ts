import type { OneclawClientConfig, OneclawResponse, PaymentRequirement, X402Signer } from "./types";
...
export class HttpClient {
  ...
  async request<T>(
    ...
    if (!res.ok) {
      ...
      return {
        data: null,
        error: { type: err.type, message: err.message, detail: err.detail },
        meta: { status: res.status },
      };
    }

    if (res.status === 204) {
      return { data: null as unknown as T, error: null, meta: { status: 204 } };
    }
